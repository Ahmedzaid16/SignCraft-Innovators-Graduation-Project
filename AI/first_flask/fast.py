...
async def upload_file(video: UploadFile = File(...)):
    print(f"Received file: {video.filename} with content type: {video.content_type}")
    if video.content_type != "video/mp4":
        raise HTTPException(status_code=400, detail="Invalid file type. Only MP4 videos are accepted.")
